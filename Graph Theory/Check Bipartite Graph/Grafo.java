...
public class Grafo {
    ...
    public static void ForwardStar(String nomeArq){
        try{
            ...
            // Fechar vetor
            pointer[totalVertices]=totalArestas+1;
            buff.close();
        } catch(IOException e){
    ...
    public static boolean isBipartido(int[] orig, int[] dest) {
        // Realizarei uma busca em largura
        BuscaLargura busca = new BuscaLargura(orig, dest);
        if(!busca.buscaBipartido()){
            return false;
        }
        return true;
    }
    public static void main(String[] args){
        ...
        if(isBipartido(pointer, arcDest)){
            System.out.println("\nO grafo é bipartido!\n");
        } else {
